...
def test_get_book_genre_returns_correct_genre(collector):
    collector.books_genre = {"Ночной дозор": "Фантастика"}
    assert collector.get_book_genre("Ночной дозор") == "Фантастика"


def test_get_books_genre_returns_full_dict(collector):
    collector.books_genre = {"Оно": "Ужасы", "Шрек": "Мультфильмы"}
    assert collector.get_books_genre() == {"Оно": "Ужасы", "Шрек": "Мультфильмы"}


def test_get_list_of_favorites_books_returns_correct_list(collector):
    collector.favorites = ["Муму", "Колобок"]
    assert collector.get_list_of_favorites_books() == ["Муму", "Колобок"]